...
# This allows running scripts directly (python project/scripts/foo.py) without
# needing to source .env first. Shell scripts also source .env, so no conflict.
_REPO_ROOT_ENV = Path(__file__).resolve().parent.parent.parent
_ENV_FILE = _REPO_ROOT_ENV / ".env"
...
CAMEL_HARD_CSV     = DATASETS_DIR / "generated_camel_hard.csv"
ESCONV_CSV         = DATASETS_DIR / "esconv_preprocessed.csv"
AUGMENTED_CSV      = DATASETS_DIR / "augmented.csv"
PAIR_CSV           = DATASETS_DIR / "generated_pair.csv"
PAIR_HARD_CSV      = DATASETS_DIR / "generated_pair_hard.csv"
...
CLASSIFIER_MAX_LENGTH     = 4096     # covers 95%+ of CAMEL rows; A6000 48GB peak ~22-25GB (safe)
...
# At 4096: 35% of generated_camel and 71% of generated_camel_hard exceeded 2048.
# At 4096: only ~5% of each exceed the budget, preserving the subtle early-escalation
# arc that camel_hard was specifically designed to capture.
