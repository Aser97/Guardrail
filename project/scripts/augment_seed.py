...
from utils import stressor_to_text, count_csv_rows, count_csv_rows_by_field, vprint
...
def _call_llm(client: PrivateClient, prompt: str, model: str, temperature: float,
               label: str = "") -> str:
...
The assistant turns must be professional: warm and empathetic, but boundaried. No emojis, no exclamation chains, no terms of endearment. 2–3 sentences per assistant turn.
...
def augment_signal_inject(
    ...
    for row in low_risk:
        ...
        try:
            ...
            new_row = _seed_row_to_master(row, new_signals, source=source_tag)
            new_row["conversation_id"] = _make_row_id()
            new_row["text"] = new_text
...
def main(argv=None) -> int:
    ...
    # ── Per-phase targets (reduced in append mode by per-source row counts) ──────
    # Each phase tags its rows with a unique `source` value so append mode can
    # subtract completed work per phase individually — fixing the bug where
    # proportional division silently set later phases to 0.
    _pt = args.per_type
    per_2a         = _pt                  # source: "lang_rewrite"
    per_2b_khp     = _pt                  # source: "persona_swap"
    esconv_swap_target = args.esconv_swap_target  # source: "esconv_swapped"
    per_2c_khp     = _pt                  # source: "signal_inject_khp"
    per_2c_esconv  = min(_pt * 2, 150)   # source: "signal_inject_esconv"
    per_2d         = _pt // 2             # source: "signal_soften"

    first_write = True  # tracks whether to write header when flushing
    if args.append and output_path.exists():
        by_source = count_csv_rows_by_field(output_path, "source")
        total_existing = sum(by_source.values())
        LOGGER.info("Append mode: %d existing rows by source: %s", total_existing, dict(by_source))
        per_2a        = max(0, per_2a        - by_source.get("lang_rewrite",         0))
        per_2b_khp    = max(0, per_2b_khp    - by_source.get("persona_swap",         0))
        esconv_swap_target = max(0, esconv_swap_target - by_source.get("esconv_swapped", 0))
        per_2c_khp    = max(0, per_2c_khp    - by_source.get("signal_inject_khp",    0))
        per_2c_esconv = max(0, per_2c_esconv - by_source.get("signal_inject_esconv", 0))
        per_2d        = max(0, per_2d        - by_source.get("signal_soften",        0))
        LOGGER.info(
            "Adjusted targets → 2a=%d  2b-khp=%d  2b-esconv=%d  2c-khp=%d  2c-esconv=%d  2d=%d",
            per_2a, per_2b_khp, esconv_swap_target, per_2c_khp, per_2c_esconv, per_2d,
        )
        first_write = False  # file exists — append without rewriting header
    ...
    # ── Incremental writer — each row is flushed immediately (crash-safe) ──────
    # first_write_ref[0] tracks whether to emit the CSV header on the first write.
    first_write_ref = [first_write]

    def on_row(row: dict) -> None:
        first_write_ref[0] = flush_phase([row], output_path, first_write_ref[0])

    # ── 2a: Language rewrite ───────────────────────────────────────────────────
    LOGGER.info("=== Phase 2a: Language Rewrite (need %d) ===", per_2a)
    rows_2a = augment_language_rewrite(seed_master + esconv_master, client, rng, per_2a, on_row=on_row)
    total_written += len(rows_2a)

    # ── 2b: Persona swap — KHP ────────────────────────────────────────────────
    LOGGER.info("=== Phase 2b: Persona Swap KHP (need %d) ===", per_2b_khp)
    rows_2b_khp = augment_persona_swap(
        seed_master, personas, client, rng, per_2b_khp, is_esconv=False, on_row=on_row
    )
    total_written += len(rows_2b_khp)

    # ── 2b: Persona swap — ESConv (mandatory; keep list in memory for 2c) ──────
    esconv_swapped: list[dict] = []
    if esconv_master and esconv_swap_target > 0:
        LOGGER.info("=== Phase 2b: Persona Swap ESConv (need %d) ===", esconv_swap_target)
        esconv_swapped = augment_persona_swap(
            esconv_master, personas, client, rng, esconv_swap_target, is_esconv=True, on_row=on_row
        )
        total_written += len(esconv_swapped)

    # ── 2c: Signal inject — KHP low-risk ──────────────────────────────────────
    LOGGER.info("=== Phase 2c: Signal Injection KHP (need %d) ===", per_2c_khp)
    rows_2c_khp = augment_signal_inject(
        seed_master, client, rng, per_2c_khp, on_row=on_row, source_tag="signal_inject_khp"
    )
    total_written += len(rows_2c_khp)

    # ── 2c: Signal inject — ESConv post-swap ──────────────────────────────────
    if esconv_swapped and per_2c_esconv > 0:
        LOGGER.info("=== Phase 2c: Signal Injection ESConv (need %d) ===", per_2c_esconv)
        rows_2c_esconv = augment_signal_inject(
            esconv_swapped, client, rng, per_2c_esconv, on_row=on_row,
            source_tag="signal_inject_esconv"
        )
        total_written += len(rows_2c_esconv)

    # ── 2d: Signal softening — KHP high-risk ──────────────────────────────────
    LOGGER.info("=== Phase 2d: Signal Softening (need %d) ===", per_2d)
    rows_2d = augment_signal_soften(seed_master, client, rng, per_2d, on_row=on_row)
